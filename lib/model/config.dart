...
class AppConfigProvider with ChangeNotifier {
  ...
  AppPathConfig? _appPathConfig;

  // 获取接口
  Locale? get locale => _locale;
  ThemeMode get themeMode => _themeMode;
  AppPathConfig? get appPathConfig => _appPathConfig;
  ...
  Future<void> _loadFromPrefs() async {
    ...
    // 初始化全局路径配置
    await PathManager.getDirPath(null).then((String appSupportPath) {
      _appPathConfig = AppPathConfig(
        supportDirPath: appSupportPath,
        versionsDirPath: p.join(appSupportPath, "versions"),
        distDirPath: p.join(appSupportPath, "dist"),
      );
    });
    notifyListeners();
  }
